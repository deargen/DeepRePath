...
class ModelXgb():
    ...
    def train(self, train_x, val_x, train_y, val_y, model_name_tosave, pt_model_file, cal=True):
        clf = xgb.XGBClassifier(seed=1, objective='binary:logistic', n_estimators=1000)
        
        try:
            pt_model = xgb.XGBClassifier()
            pt_model.load_model(pt_model_file)  # load data
            pt_model = pt_model.get_booster()
        except:
            print('\npt_model = None\n')
            pt_model = None
        
        try:
            clf.fit(train_x, train_y, eval_set=[(val_x, val_y)], verbose=True, eval_metric='auc', early_stopping_rounds=100, xgb_model=pt_model)
